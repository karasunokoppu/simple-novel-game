...
use crate::{
    despawn_screen,
    game_states::in_game::{ImageAssets, InGameState, NovelGameStates, StoryImageList, StoryWallPaperList, WallpaperAssets},
};

pub fn draw_image_plugin(app: &mut App) {
    app
        .add_systems(OnEnter(InGameState::Draw), setup_draw_image)
                ...
                for image_data in image_datas.iter(){
                    if display_image.story_scene_id == novel_game_states.current_story_id as u32{
                        ...
                        if display_image.left_image_scene_id == image_data.image_id {
                            left_node = (
                                ImageNode::new(image_assets.images.get(&display_image.left_image_scene_id).unwrap().clone()),
                                Transform::from_xyz(0.0, 0.0, 0.0).with_scale(Vec3::new(image_data.scale.0, image_data.scale.1, 1.0)),
                                Node {
                        ...
                        if display_image.center_image_scene_id == image_data.image_id {
                            center_node = (
                                ImageNode::new(image_assets.images.get(&display_image.center_image_scene_id).unwrap().clone()),
                                Transform::from_xyz(0.0, 0.0, 0.0).with_scale(Vec3::new(image_data.scale.0, image_data.scale.1, 1.0)),
                                Node {
                                    width: Val::Percent(100.0 / 3.0),
                        ...
                        if display_image.right_image_scene_id == image_data.image_id {
                            right_node = (
                                ImageNode::new(image_assets.images.get(&display_image.right_image_scene_id).unwrap().clone()),
                                Transform::from_xyz(0.0, 0.0, 0.0).with_scale(Vec3::new(image_data.scale.0, image_data.scale.1, 1.0)),
                                Node {
                                    width: Val::Percent(100.0 / 3.0),
        ...
        if *story == novel_game_states.story{
            for wallpaper_data in wallpaper_datas.iter() {
                for wallpaper in wallpapers.iter() {
                    if wallpaper_data.story_id == novel_game_states.current_story_id as u32 && wallpaper.image_id == wallpaper_data.image_id{
                        wallpaper_node = (
                            ImageNode::new(wallpaper_assets.images.get(&wallpaper.image_id).unwrap().clone()),
                            Transform::from_xyz(0.0, 0.0, 0.0),
                            Node {
                                width: Val::Percent(100.0),
                                height: Val::Percent(100.0),
